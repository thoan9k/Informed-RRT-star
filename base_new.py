...
class RRTmap:
    def __init__(self, start, goal, mapdimensions, obsdim, obsnum, Mapwindowname='RRT path planning', type="RRT_star"):
        self.type = type
        self.start = start
        self.goal = goal
...
class RRTgraph:
    def __init__(self, start, goal, mapdimensions, obsdim, obsnum, speed=0,type='RRT_star', Mapwindowname='RRT path planning'):
    ...
    def drawmap(self, obstacles):
        # vẽ điểm bắt đầu và điểm đích
        pygame.draw.circle(self.surface, self.green, self.start, self.nodeRadian + 10, 0)
        pygame.draw.circle(self.surface, self.red, self.goal, self.nodeRadian + 20, 1)
        # Vẽ các trướng ngại vật hình vuông
        self.drawObs(obstacles)
    ...
    def drawpath(self, coords):
        for coord in coords:
            pygame.draw.circle(self.surface, self.red, coord, self.nodeRadian + 3, self.nodeThickness)

    def drawObs(self, obstacles):
        obstacleslist = obstacles.copy()
        while len(obstacleslist) > 0:  # vẽ các hình chữ nhật bên trong map đã tạo(nền trắng)
            obstacle = obstacleslist.pop(0)
            pygame.draw.rect(self.surface, self.grey, obstacle)
    ...
    def Cost(self, node):
        ...
        while current_node!=0:
            p_current_node = self.parent[current_node]
            c+= self.distance(current_node, p_current_node)
