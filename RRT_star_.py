...
goal = (400, 500)
dimensions = (600, 600)
obsdim = 60 # kích thước đã phóng đại
adjust_obs = -35
obsnum = 30
iteration_max = 1000
...
def main(obs, graph, map_):
    ...
    while iteration <= iteration_max:
        ...
        if iteration % 10 == 0:
            ...
            pygame.draw.circle(map_.map, map_.red, (x[-1], y[-1]), map_.nodeRadian, map_.nodeThickness)
            ...
            pygame.draw.circle(map_.map, map_.grey, (x[-1], y[-1]), map_.nodeRadian, map_.nodeThickness)
            pygame.draw.line(map_.map, map_.blue, (x[-1], y[-1]),
                             (x[parent[-1]], y[parent[-1]]), map_.edgeThickness)
            if graph.number_of_nodes() - n == 1:
                new_node = graph.number_of_nodes() - 1
        ...
        if graph.path_to_goal():
            ...
            if new_cost < oldcost:
                ...
                if graph.r1 != 0:
                    ...
                    map_.map.blit(rotated_surface, rotated_rect.topleft)
                    pygame.display.update()  
...
if __name__ == '__main__':
    ...
    i_ = 0
    #print(obs_)
    map_.drawmap(graph_.getTrueObs(obs_, adjust_obs))
    pygame.display.update()
    time.sleep(1)
    while not result:
        # if i_ > 0:
